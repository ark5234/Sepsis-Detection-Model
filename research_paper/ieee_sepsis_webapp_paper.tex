...
\usepackage[font=it, labelsep=space]{caption}
\usepackage{setspace}
\setstretch{1.15}
\raggedbottom
...
% ── Title ─────────────────────────────────────────────────────────────────────
\title{DPCT: A Dual-Path Clinical Transformer for Early Sepsis Detection in ICU Time Series via Time-Decay Embeddings, Bidirectional Cross-Attention, and Clinical Gate Supervision}

\author{%
  \IEEEauthorblockN{Vikrant Kawadkar (Student Researcher)}
  \IEEEauthorblockA{\textit{Department of Computer and Communication Engineering} \\
  \textit{Manipal University Jaipur}, India \\
  vikrantkawadkar2099@gmail.com}
  \and
  \IEEEauthorblockN{Dr. Pramod Singh Rathore (Faculty Guide)}
  \IEEEauthorblockA{\textit{Department of Computer and Communication Engineering} \\
  \textit{Manipal University Jaipur}, India \\
  pramod.rathore@jaipur.manipal.edu}
}
...
\subsection{\textbf{Rule-Based Early Sepsis Warning Systems}}
\label{sec:rel:rules}
...
\subsection{\textbf{Temporal Deep Learning for Clinical Data}}
\label{sec:rel:dl}
...
\subsection{\textbf{Missing-Data Handling in Clinical Time Series}}
...
\subsection{\textbf{Transformer-Based Clinical Risk Scoring}}
...
% ════════════════════════════════════════════════════════════════════════════
\section{\textbf{Dataset and Leakage-Correction Protocol}}
\label{sec:data}
% ════════════════════════════════════════════════════════════════════════════

\subsection{\textbf{PhysioNet Computing in Cardiology Challenge 2019}}
...
\subsection{\textbf{Patient-Level Split Protocol}}
...
% ════════════════════════════════════════════════════════════════════════════
\section{\textbf{Dual-Path Sequence Preparation}}
\label{sec:prep}
% ════════════════════════════════════════════════════════════════════════════
...
% ════════════════════════════════════════════════════════════════════════════
\section{\textbf{Dual-Path Clinical Transformer}}
\label{sec:model}
% ════════════════════════════════════════════════════════════════════════════
...
\subsection{\textbf{Path-Specific Encoders}}
\label{sec:encoders}
...
\subsection{\textbf{Time-Decay Lab Embedding}}
\label{sec:tdecay}
...
\begin{itemize}
  \item $\lambda = 0.05\,\mathrm{h}^{-1}$ is the fixed exponential decay rate
        (half-life $\approx 14$\,h, confirmed optimal via a brief sensitivity sweep across $\lambda \in \{0.01, 0.05, 0.1\}$);
...
\subsection{\textbf{Bidirectional Cross-Attention Fusion}}
\label{sec:fusion}
...
\begin{center}
  ...
  \textit{Equations 2 \& 3: Bidirectional Fusion}
\end{center}
\vspace{1em}
%
where $\mathrm{MHA}(Q, K, V)$ denotes scaled dot-product multi-head attention
with 4 heads.
In Equation 2 (Vital Fusion), the vital stream \emph{queries} the lab stream, absorbing
relevant laboratory context; in Equation 3 (Lab Fusion), the lab stream queries the
...
\begin{center}
  ...
  \textit{Equation 4: Fused Sequence}
\end{center}
\vspace{1em}

\subsection{\textbf{Clinical Threshold Gate}}
\label{sec:gate}
...
\begin{center}
  ...
  \textit{Equation 5: Clinical Rules}
\end{center}
\vspace{1em}
%
...
\begin{center}
  ...
  \textit{Equation 6: Gate Activation}
\end{center}
\vspace{1em}
%
...
(Equation 7: Attention Pooling), allowing the model to assign additional attention weight to
...
\subsection{\textbf{Attention-Weighted Pooling and Classification}}
\label{sec:pool}
...
\begin{center}
  ...
  \textit{Equation 7: Attention Pooling}
\end{center}
\vspace{1em}
%
...
\subsection{\textbf{Model Size and Training Configuration}}
\label{sec:training}
...
\subsection{\textbf{5-Fold Cross-Validation}}
...
We note that while this introduces an inherent advantage in temporal dynamics for \dpct{}, the ablation study (\sect{sec:results:ablation}, Base Transformer ROC-AUC = 0.921) confirms that the performance gains stem from the proposed architectural innovations and not just the sequential format.
...
While XGBoost achieves marginally higher PR-AUC and F1 at the default threshold, this reflects its conservative operating point optimised for aggregated features; \dpct{}'s recall advantage (0.703 vs 0.651) is clinically more significant.

\subsection{\textbf{Held-Out Test Set Performance}}
\label{sec:results:test}

The held-out test predictions were generated by an ensemble averaging the output probabilities of all five fold models (each using threshold $\tau = 0.88$) to ensure robust evaluation and leverage the full training distribution.
This ensemble was evaluated on the completely held-out 20\% test
cohort ($n = 8\,068$ patients, including 586 sepsis-positive cases), which was
strictly excluded from all training, validation, and threshold tuning.
This ensemble approach naturally yields slightly higher performance than individual cross-validation folds.
...
\subsection{\textbf{Decision Threshold Optimisation}}
...
discriminative margin, significantly reducing false-positive alarm fatigue.
While this conservatively high threshold is critical for deployment, it presents a trade-off by potentially missing borderline or early-onset cases. In a live clinical setting, this threshold should be adjusted downwards based on local ICU staffing capacity and baseline prevalence.
...
To quantify the contribution of each architectural component, we conducted an ablation study (\tab{tab:ablation}) evaluating progressive additions to a base Transformer model.
Removing the dual-path design entirely (concatenating all features) resulted in the largest performance drop, confirming the necessity of modality-specific processing. The Time-Decay Embedding and Bidirectional Cross-Attention sequentially improved both ROC-AUC and Recall. Finally, while removing the Clinical Threshold Gate yielded only a marginal decrease in overall ROC-AUC, its inclusion notably improved Recall ($0.685 \to 0.703$) and is retained primarily for its critical interpretability benefits (\sect{sec:interp:gate}).

\begin{table}[!t]
  \centering
  \caption{Ablation Study on DPCT Architectural Components (5-Fold CV Mean)}
  \label{tab:ablation}
  \small
  \renewcommand{\arraystretch}{1.1}
  \resizebox{\columnwidth}{!}{%
  \begin{tabular}{@{}lcc@{}}
    \toprule
    \textbf{Model Variant} & \textbf{ROC-AUC} & \textbf{Recall} \\
    \midrule
    Base Transformer (no dual-path) & $0.921$ & $0.612$ \\
    + Dual-path (no time-decay) & $0.935$ & $0.655$ \\
    + Time-decay embedding & $0.944$ & $0.680$ \\
    + Cross-attention fusion (DPCT w/o Gate) & $0.949$ & $0.685$ \\
    \textbf{+ Clinical gate (Full DPCT)} & $\mathbf{0.952}$ & $\mathbf{0.703}$ \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
...
% ════════════════════════════════════════════════════════════════════════════
\section{\textbf{Interpretability Analysis}}
\label{sec:interp}
% ════════════════════════════════════════════════════════════════════════════

\subsection{\textbf{Statistical Significance of Performance Gains}}
...
\begin{table}[!t]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \textbf{Comparison}       & \textbf{McNemar $p$}
                              ...
                              & \textbf{Sig.}   \\
    \midrule
    \dpct{} vs.\ Random Forest & $< 0.0001$ & $+0.029$ & \checkmark \\
    \dpct{} vs.\ XGBoost       & $< 0.0001$ & $+0.023$ & \checkmark \\
    \dpct{} vs.\ DNN (MLP)     & $< 0.0001$ & $+0.056$ & \checkmark \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
...
McNemar's test, and the bootstrap AUC differences demonstrate consistent margins (e.g., $+0.023$ over XGBoost).
These results confirm that \dpct{}'s advantage is systematic and not the product
of stochastic variation.

\subsection{\textbf{Temporal Attention Analysis}}
\label{sec:interp:attn}

\fig{fig:attn_line} presents the mean~$\pm 1\sigma$ temporal attention weight
$\alpha_t$ (Equation 7: Attention Pooling) pooled over all test patients, stratified by sepsis
label.
...
\subsection{\textbf{Clinical Threshold Gate Weights}}
...
\subsection{\textbf{SHAP Analysis of the XGBoost Baseline}}
...
% ════════════════════════════════════════════════════════════════════════════
\section{\textbf{System Architecture and Deployment}}
\label{sec:system}
% ════════════════════════════════════════════════════════════════════════════

To facilitate clinical translation, the trained \dpct{} model is open-sourced and packaged as a production-grade \texttt{FastAPI} web service, available at \url{https://github.com/Vikrant/sepsis-detection-webapp}. The system provides a bedside manual-entry interface for real-time inference (requiring $<100$\,ms per patient on standard CPU hardware) and supports batch processing. The web application dynamically renders the learned temporal attention timeline and active clinical rule violations to clinicians. Additionally, it integrates a Retrieval-Augmented Generation (RAG) AI assistant powered by Google Gemini 2.5 Flash, which contextualises model predictions against Sepsis-3 guidelines while maintaining strict patient-specific conversational memory.
...
\subsection{\textbf{Clinical Significance of Recall-First Optimisation}}
...
\subsection{\textbf{Comparison with Published Results}}
...
in \textit{Thirty-Second AAAI Conference on Artificial Intelligence}, 2018.
